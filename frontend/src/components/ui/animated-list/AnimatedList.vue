...
async function startLoop() {
  // `children` is typed as string | VNodeArrayChildren | RawSlots upstream;
  // only the array form is iterable, so narrow before use.
  const children = slots.default?.()[0]?.children;
  const notifications = Array.isArray(children) ? children : [];
